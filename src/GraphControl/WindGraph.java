...
public class WindGraph {
	...
	private int V;
	private int E;
	private HashMap<Location, WindDataNode> wind;
	...
	public void setupGraph() {
		...
		wind = DataParser.getWindData();

		locations = new Location[wind.keySet().size()];

		wind.keySet().toArray(locations);
	...
	private Location getSourceLocation(Location l) {
		for (Location loc : wind.keySet())
			if (loc.equals(l))
				return loc;
		return null;
	}

	public ArrayList<WindDataNode> getInterferenceZone(Location l) {
		l = getSourceLocation(l);
		ArrayList<WindDataNode> retval = new ArrayList<>();
		retval.add(wind.get(l));
		for (Location loc : BFS(l, 200)) {
			retval.add(wind.get(loc));
		}
